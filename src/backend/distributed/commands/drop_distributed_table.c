...
static void MasterDropDistributedTableMetadataOnWorkers(Oid relationId, char *schemaName,
														char *tableName);
...
PG_FUNCTION_INFO_V1(master_drop_distributed_table_metadata_on_workers);
 ...
 * master_drop_distributed_table_metadata_on_workers() followed by
...
/*
 * master_drop_distributed_table_metadata_on_workers removes the entry of the specified distributed
 * table from pg_dist_partition and drops the table from the workers if needed.
 */
Datum
master_drop_distributed_table_metadata_on_workers(PG_FUNCTION_ARGS)
{
	Oid relationId = PG_GETARG_OID(0);
	text *schemaNameText = PG_GETARG_TEXT_P(1);
	text *tableNameText = PG_GETARG_TEXT_P(2);
	...
	MasterDropDistributedTableMetadataOnWorkers(relationId, schemaName, tableName);
...
/*
 * MasterDropDistributedTableMetadataOnWorkers drops the table and removes
 ...
 */
static void
MasterDropDistributedTableMetadataOnWorkers(Oid relationId, char *schemaName,
											char *tableName)
{
