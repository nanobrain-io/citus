...
CREATE OR REPLACE FUNCTION master_remove_partition_metadata(logicalrelid regclass,
                                                   schema_name text,
                                                   table_name text)
    RETURNS void
    LANGUAGE C STRICT
...
COMMENT ON FUNCTION master_remove_partition_metadata(logicalrelid regclass,
                                                       schema_name text,
                                                       table_name text)
    IS 'delete the partition metadata of the distributed table';

CREATE OR REPLACE FUNCTION master_drop_distributed_table_metadata_on_workers(logicalrelid regclass,
                                                   schema_name text,
                                                   table_name text)
    RETURNS void
    LANGUAGE C STRICT
    AS 'MODULE_PATHNAME', $$master_drop_distributed_table_metadata_on_workers$$;
COMMENT ON FUNCTION master_drop_distributed_table_metadata_on_workers(logicalrelid regclass,
                                                       schema_name text,
                                                       table_name text)
    IS 'drops the table and removes all the metadata beloning the distributed table in the worker nodes with metadata.';
...
BEGIN
    ...
    LOOP
        ...
        PERFORM master_drop_distributed_table_metadata_on_workers(v_obj.objid, v_obj.schema_name, v_obj.object_name);
