...
step "s2-view-dist"
{
	SELECT query, query_hostname, query_hostport, initiator_node_hostname, initiator_node_port, state, wait_event_type, wait_event, usename, database_name FROM citus_dist_stat_activity ORDER BY query DESC;
...
step "s3-view-worker"
{
	SELECT query, query_hostname, query_hostport, initiator_node_hostname, initiator_node_port, state, wait_event_type, wait_event, usename, database_name FROM citus_worker_stat_activity ORDER BY query DESC;
}
